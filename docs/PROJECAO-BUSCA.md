...
Implementação de 08/09/2026 em `backend/bigbase/canonical_search.py`. Contrato original: `canonical-search-2026-09-08.2`; a MAIN agora prepara `canonical-search-2026-09-09.1`, conforme [BUSCA-CAMPOS-CANONICOS.md](BUSCA-CAMPOS-CANONICOS.md). Este bloco usa somente dados sintéticos e transporte HTTP simulado nos testes. Não criou índice, serviço ou daemon e não conectou ao Elasticsearch atual.
...
Campos adicionais escalares com definição vinculada agora têm projeção e filtros conforme [BUSCA-CAMPOS-CANONICOS.md](BUSCA-CAMPOS-CANONICOS.md); referências e caminhos desconhecidos continuam fora da busca. Valores com NUL, surrogates isolados, mais de 8.192 bytes UTF-8 ou estruturas não contempladas são omitidos da busca e contabilizados; permanecem completos no PostgreSQL. Não se substitui caractere nem se trunca um valor para fingir que ele foi indexado. O documento informa `omitted`, que a operação deve monitorar durante o piloto.

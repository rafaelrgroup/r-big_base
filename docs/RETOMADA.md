...
São excluídos histórico Git, dependências reinstaláveis, caches, runtimes de ensaio, banco PostgreSQL sintético, workspace temporário do navegador, credenciais SSH/Codex e os bancos/snapshot de origem. O build atual do painel e as evidências selecionadas são incluídos. Novos arquivos privados fora da seleção exigem revisão explícita do inventário. O limite atual é 128 MiB por pacote e 32 MiB por arquivo; excedê-lo interrompe o processo sem omitir dados silenciosamente.

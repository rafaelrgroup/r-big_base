...
from .canonical_search_availability import custom_filter_availability
...
def verify_catalog(c):
    ...
    identity = getattr(c,'bigbase_deployment_identity',None)
    if identity is not None:
        from .deployment import DeploymentIdentity
        if not isinstance(identity,DeploymentIdentity):raise CanonicalError('Explicit deployed catalog identity required')
        identity.verify(info)
    elif (info['environment'] != 'synthetic' or info['database'] != 'bigbase_test'
...
def mutate_catalog(repository, *, actor_id, request_key, body, field_id=None, if_match=None):
    ...
    with repository.connection() as c:
        ...
        receipt = {'operation_id': str(operation), 'definition': definition,
                   ...
                   'catalog_environment': 'postgresql_'+info['environment'], 'production_connected': info['environment']=='production'}
...
def install_canonical_catalog(app, reads, *, store, security, auth):
    ...
    def available(request, mutate=False):
        ...
        if reads is None or (mutate and not reads.writes_enabled):
            failure(503, 'CANONICAL_CATALOG_DISABLED', 'Catálogo PostgreSQL não configurado.')
        try:
            reads.verify()
            yield user
        except (psycopg.Error, CanonicalError, ValueError):
            failure(503, 'CANONICAL_CATALOG_UNAVAILABLE', 'Catálogo PostgreSQL indisponível; repita com a mesma chave.')
        ...
        with available(request):
            with reads.repository.connection() as c:
                ...
                return exact_response({'items': [decode(row['definition_json']) for row in rows[:limit]],
                    ...
                    'catalog_environment': 'postgresql_'+info['environment'], 'search_state': 'pending'})
        ...
        with available(request):
            with reads.repository.connection() as c:
                ...
                return exact_response({'items': [{'definition': decode(row['definition_json']), 'definition_sha256': row['definition_sha256']} for row in rows[:limit]],
                    'next_after': rows[limit-1]['version'] if len(rows)>limit else None, 'catalog_environment': 'postgresql_'+info['environment']})

    @app.get('/api/v1/canonical/fields/{field_id}/versions/{version}/search-availability')
    def search_availability(field_id: str, version: int, request: Request):
        with available(request):
            if (not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9_.:-]{0,159}', field_id)
                    or not 1 <= version <= 2**63-1 or request.query_params):
                failure(422, 'INVALID_SEARCH_AVAILABILITY_REQUEST', 'Informe somente campo e versão histórica.')
            provider = getattr(reads, 'search_availability_provider', None)
            if provider is None:
                failure(503, 'SEARCH_AVAILABILITY_DISABLED', 'Disponibilidade de filtros não configurada.')
            with reads.repository.connection() as c:
                info = verify_catalog(c)
                # This preparation route is never enabled for a deployed database.
                if info['environment'] != 'synthetic':
                    failure(503, 'SEARCH_AVAILABILITY_DISABLED', 'Disponibilidade de filtros não configurada.')
                row = c.execute('SELECT definition_json,definition_sha256 FROM field_catalog_versions WHERE field_id=%s AND version=%s', (field_id, version)).fetchone()
                if row is None:
                    failure(404, 'CANONICAL_FIELD_VERSION_NOT_FOUND', 'Versão de campo não encontrada.')
                definition = decode(row['definition_json'])
                if (not isinstance(definition, dict) or definition.get('id') != field_id
                        or type(definition.get('version')) is not int or definition.get('version') != version
                        or digest(definition) != row['definition_sha256']):
                    raise CanonicalError('Historical definition mismatch')
                metadata = {'field_id': field_id, 'field_definition_version': version,
                    'field_definition': definition, 'field_definition_sha256': row['definition_sha256'],
                    'custom_field': {'contract': PG_FIELD_CONTRACT, 'field_id': field_id, 'version': version},
                    'catalog_deployment_id': info['deployment_id']}
                # Provider receives a selector, not request/credentials. Its context
                # must be independently pinned, and its receipt internally verified.
                from .canonical_search import custom_selector
                try:
                    context = provider(custom_selector(metadata))
                    if (not isinstance(context, dict)
                            or set(context) != {'target', 'coverage_cut_sha256', 'receipt'}):
                        raise ValueError('Invalid internal context')
                    result = custom_filter_availability(metadata, **context)
                except Exception:
                    # Provider failures may contain infrastructure or secret data.
                    failure(503, 'SEARCH_AVAILABILITY_UNAVAILABLE', 'Prova de disponibilidade indisponível.')
                response = exact_response({'availability': result,
                    'catalog_environment': 'postgresql_synthetic', 'search_execution_enabled': False})

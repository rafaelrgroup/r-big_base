...
try{
 ...
 await page.getByRole('button',{name:'Campos adicionais',exact:true}).click();
 await page.getByRole('button',{name:'Adicionar',exact:true}).click();
 await page.locator('dialog').getByLabel('Nome',{exact:true}).fill('Inteiro canônico sintético');
 await page.locator('dialog').getByLabel('Tipo',{exact:true}).selectOption('integer');
 ...
 await expect(page.getByText('Inteiro canônico sintético',{exact:true})).toBeVisible();

...
export function CanonicalCatalog({ api, canAdmin, deployed = false }: { api: API; canAdmin: boolean; deployed?: boolean }) {
  ...
  return <section aria-label={deployed ? 'Catálogo de campos' : 'Catálogo PostgreSQL sintético'} className="card">
    <h3>{deployed ? 'Definições de campos adicionais' : 'Definições de campos no PostgreSQL sintético'}</h3>
    <p>{deployed ? 'Versões anteriores e observações permanecem preservadas. Indexação de campos adicionais pendente.' : 'Catálogo separado do SQLite local. Versões anteriores e observações permanecem preservadas. Indexação pendente.'}</p>
    ...
    {canAdmin && !needsOTP && <form onSubmit={save}>
      <fieldset disabled={busy}>
        ...
        {!selected && <>
          ...
          <label>Tipo da definição PostgreSQL<select aria-label="Tipo da definição PostgreSQL" value={type} onChange={e => setType(e.target.value)}>{['text','integer','decimal','boolean','date','enum','url','reference'].map(t => <option key={t}>{t}</option>)}</select></label>
          <label>Escopo da definição PostgreSQL<select aria-label="Escopo da definição PostgreSQL" value={scope} onChange={e => setScope(e.target.value)}><option value="both">Pessoa e empresa</option><option value="person">Pessoa</option><option value="company">Empresa</option></select></label>

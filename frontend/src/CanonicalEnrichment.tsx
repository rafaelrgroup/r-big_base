...
export function CanonicalEnrichment({ api, collection, entity, source, external, onWritten, canValidate }: {
  api: API; collection: string; entity: any; source: string; external: string; onWritten: (id: string, source: string, external: string) => Promise<void>; canValidate: boolean;
}) {
  ...
  const [entries, setEntries] = useState<Entry[]>([empty()]);
  ...
  return <section aria-label="Enriquecimento canônico">
    <h3>{entity ? 'Acrescentar observações à ficha' : 'Criar cadastro sintético'}</h3>
    <p>{entity ? `A escrita exige a versão ${entity.version} da ficha aberta.` : 'A identidade de origem deve ser nova. Para alterar um cadastro, abra a ficha primeiro.'} Use somente dados fictícios.</p>
      ...
      <fieldset disabled={busy}>
        ...
        {entries.map((row, i) => <fieldset key={i} aria-label={`Observação ${i + 1}`}>
          <legend>Observação {i + 1}</legend><div className="form-grid">
            ...
            {row.kind === 'custom' && <>
              <label>Catálogo da definição<select aria-label="Catálogo da definição" value={row.catalogEnvironment} onChange={e => { edit(i, 'catalogEnvironment', e.target.value); edit(i, 'normalization', 'literal'); edit(i, 'fieldId', ''); edit(i, 'fieldVersion', ''); if (e.target.value === 'postgresql') reloadPG(); }}><option value="local">SQLite local — contrato anterior</option><option value="postgresql">PostgreSQL sintético — transacional</option></select></label>
        ...
        <button type="button" disabled={entries.length >= 100} onClick={() => setEntries(old => [...old, empty()])}>Adicionar observação</button>{' '}
